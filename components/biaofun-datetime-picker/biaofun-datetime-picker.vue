...
<template>
	<view class="datatime">
		<picker mode="multiSelector" :range="range" range-key="text" @change="change" @columnchange="columnchange" :value="value"
		 :disabled="disabled">
			<view class="color333" :class="{ placeholder: !dateStr }">
	...
	export default {
		...
		methods: {
			...
			setDateStr(date) {
				...
				// 3. Y年m月d日
				// 4. Y年m月d日 H时i分
				this.dateStr = this.formats.replace(/Y|m|d|H|i|s/ig, function(matches) {
					return ({
						Y: dt.YYYY,
						m: dt.M,
						d: dt.D,
			...
			columnchange(event) {
				...
				if (columnIndex == 0 || columnIndex == 1 || columnIndex == 2) {
					...
					// 更新时数据
					this.setHourData(this.range[0][this.value[0]].number, this.range[1][this.value[1]].number, this.range[2][this.value[
						2]].number);
					// 更新 Hour Value
					let hourIndex = this.range[3].findIndex(hour => {
				...
				// 更新分数据
				this.setMinuteData(this.range[0][this.value[0]].number, this.range[1][this.value[1]].number, this.range[2][this.value[
					2]].number, this.range[3][this.value[3]].number);
				// 更新 Minute Value
				let minuteIndex = this.range[4].findIndex(minute => {
