<template>
	<view class="content">
		<view id="search" class="flex-between w-calc40 pd-20 bg-white borderb">
			<view class="inputs">
				<input @confirm="search" placeholder-class="plh" maxlength="10" name="search" confirm-type="search" v-model="keyword"
			...
			<swiper-item v-for="(item, i) in categoryData" :key="i" class="my-swiper-item">
				<mescroll-uni :ref="'mescrollRef' + i" @init="mescrollInit" @down="downCallback(i)" @up="upCallback(i)" :safearea="true"
				 :fixed="false" :bottombar="false">
					<view class="item bg-white borderb" v-for="(items, i) in item.listData" :key="i" @click="go('packages/orderdetail/index?orderId=' + items.ordersid)">
						<view class="status">
		...
		<uni-popup ref="popup" type="top">
			<view class="box bg-white">
				<view class="pd-30">
					...
					<view class="dflex">
						<view>
							<MyPicker mode="date" placeholder="选择时间" :value="d1" @change="getD1"></MyPicker>
						</view>至
						<view>
